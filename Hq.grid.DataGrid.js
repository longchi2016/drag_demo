Hq.ns("Hq.grid.DataGrid");
/**
 * 表的定义
 */
Hq.grid.DataGrid = function(gridSetting) {
	if ($.isEmptyObject(gridSetting))
		return;
	// 事件类型常量
	Hq.grid.DataGrid.ONCHANGEDATA = "onchangedata";
	// 控制字段扩展前缀
	Hq.grid.DataGrid.PREFIXEXPAND = "SNAME_";
	Hq.grid.DataGrid.PREFIXEXPAND_C = "C_";
    Hq.grid.DataGrid.defaultCellColor = "bl";
	Hq.grid.DataGrid.DOWNLOADFILEURL = "commons/fileserver/downloadFile.do?tokenid="+tokenID;
	Hq.grid.DataGrid.UPLOADFILEURL = "commons/fileserver/uploadFile.do?";
	/**
	 * grid 的默认的配置
	 */
	this.setting = {
	...
	function initEvents(setting) {
		...
		Hq.EventManager.addEventListener({
		...
		// 事件绑定 
		Hq.EventManager.addEventListener({
					eventType : Hq.BaseEvent.eventType.LOADDATA,
					handler : function(e) {
						if (e.isSelf == true) {
							Hq.tools.apply({
		...
		Hq.EventManager.addEventListener({
					eventType : Hq.BaseEvent.eventType.BEFORELOADDATA,
					handler : function(e) {
						if (e.isSelf == true) {
							Hq.tools.apply({
		...
		Hq.EventManager.addEventListener({
					eventType : Hq.BaseEvent.eventType.EDITSTART,
					handler : function(e) {
						if (e.isSelf == true) {
							if (e.currentRow["C_STATUS"] == "0" && e.hqDataTable.option("canModify") == false) {
								Hq.Logger 	.info(this.id
												+ "grid停止编辑:状态为原始的，并canModify 为fale 不能修改");
								return false;
							}
							if (e.hqDataTable.option("editable") ==  false) {
								Hq.Logger .info(this.id + "grid停止编辑:当前setting editable 为false");
								return false;
							}
							if (e.currentRow["ROWSECU"] == "2") {
								Hq.Logger .info(this.id + "grid停止编辑:当前行权限为2不能修改");
								return false;
							}
							//分组行不能编辑
							if(me.getGrid().groupColumnsList && e.currentRow["level"] < me.getGrid().groupColumnsList.length - 1){
								Hq.Logger .info(this.id + "grid停止编辑:当前行为分组行不能修改");
								return false;
							}
							//列限制
		...
		Hq.EventManager.addEventListener({
					eventType : Hq.BaseEvent.eventType.EDITEND,
					handler : function(e) {
						if (e.isSelf == true) {
							Hq.tools.apply({
		...
		Hq.EventManager.addEventListener({
					eventType : Hq.grid.DataGrid.ONCHANGEDATA,
					handler : function(e) {
						if (e.isSelf == true) {
							Hq.tools.apply({
		...
		Hq.EventManager.addEventListener({
					eventType : Hq.BaseEvent.eventType.SAVEDATA,
					handler : function(e) {
						if (e.isSelf == true) {
							Hq.tools.apply({
		...
		if ($.type(setting.callback.oneditstart) === 'object') {
			for (var colName in setting.callback.oneditstart) {
				Hq.EventManager.addEventListener({
					eventType : colName + '@' + Hq.BaseEvent.eventType.EDITSTART,
					handler : function(e) {
						if (e.isSelf == true) {
							Hq.tools.apply({
		...
		(function(events) {
			$.each(events, function(index, eventName) {
				me.setting.dataTable.onEvent(eventName, function() {
					Hq.EventManager.dispatchEvent(new Hq.grid.DataGridEvent({
						eventElement : setting.me,
						eventType : eventName,
						hqDataTable : setting.me,
		...
		$.each(cols, function(index, col) {
					col.oneditend = function() {
						...
						var col = this;
						// 自定义onchangedata事件
						var newEvent = new Hq.grid.DataGridEvent({
									eventElement : setting.me,
									eventType : Hq.BaseEvent.eventType.EDITEND,
									hqDataTable : setting.me,
						...
						Hq.EventManager.dispatchEvent(newEvent);

						Hq.EventManager.dispatchEvent($.extend({}, newEvent, {
									eventType : Hq.grid.DataGrid.ONCHANGEDATA,
									changeTyep : "update"

								}));
						Hq.EventManager.dispatchEvent($.extend({}, newEvent, {
									eventType : col.name + '@' + Hq.BaseEvent.eventType.EDITEND
								}));
					};
					col.oneditstart = function() {
						...
						var a = Hq.EventManager.dispatchEvent(newEvent);
						if (a != null) {
							return a;
						}
						return Hq.EventManager.dispatchEvent($.extend({},
								newEvent, {
									eventType : col.name + '@' 	+ Hq.BaseEvent.eventType.EDITSTART
								}));
					};
				});
	};
	...
	function beginDrawTable(recordset, setting) {
		Hq.EventManager.dispatchEvent(new Hq.grid.DataGridEvent({
					eventElement : setting.me,
					eventType : Hq.BaseEvent.eventType.BEFOREDRAW,
					hqDataTable : setting.me,
		...
		Hq.EventManager.dispatchEvent(new Hq.grid.DataGridEvent({
					eventElement : setting.me,
					eventType : Hq.BaseEvent.eventType.COMPLETED,
					hqDataTable : setting.me,
					gridSetting : setting.gridSetting,
					ltDataTable : grid
				}));
	...
	// 得到适配器
	this.adapter = Hq.adapter.AdapterManager.getAdapter(this);
	...
	if (this.setting.localConfigration == false) {
		...
		this.getConfigration(gridSetting, function(serviceConfig) {
					...
					Hq.grid.DataGrid.superclass.constructor
							.call(me, me.setting);
					me.init();
					initGrid(me.setting);
		...
		Hq.grid.DataGrid.superclass.constructor.call(this, this.setting);
		this.init();
...
Hq.extend(Hq.grid.DataGrid, Hq.grid.Panel, {
	xType : 'grid',
	className : 'grid',
	...
	init : function() {
		...
		Hq.EventManager.removeAllEventListener(this);
	...
	initFastQueryField : function(){
		...
		if(this.setting.fastQuery==true){
			this.queryFieldId = Hq.getUniqueId();
			tbar.push({
				float : 'l',
				sortnum : 0,
	...
	advancedSearchField : function(fn){//fn为回调
		...
		Hq.create(advancedSearchSetting);
	},
	/*
	 * 初始化工具条
	 */
	...
	_getShowStatusPanel : function() {
		if(this.showFootbar ==null){
			var flag= Hq.grid.DataGrid.superclass._getShowStatusPanel.call(this);
	...
	_checkColLimit : function(e) {
		var columnLimitList = e.hqDataTable.setting.columnLimitList;
	...
	_mergeSting : function(userConfig, serviceConfig) {
		...
		var tempTab = Hq.getCmp(this.renderId);
		if (tempTab != null) {
		...
		if ($.type(serviceConfig) === "object") {
			...
			this.setting.grid = Hq.JSON.stringify(serviceConfig);
		} else if ($.type(serviceConfig) === "string") {
			_setting = Hq.JSON.parse(serviceConfig);
			this.setting.grid = serviceConfig;
		} else {  //
			// 创建一个默认的grid
			if(this.setting.grid== null){
				_setting = this.adapter.parseDefine(userConfig);
			   this.setting.grid  =this.setting.grid || Hq.JSON.stringify(_setting);
			}
...
/*		if (this.setting.editable == false) {
		...
		if ($.isArray(options)) {
			for (var i = 0; i < options.length; i++) {
				// 查询col
				var cols = Hq.tools.query(options[i].key, this.setting.cols);
				for (var j = 0; j < cols.length; j++) {
					options[i].value = options[i].value || {};
					Hq.merge(cols[j], options[i].value);
				}
			}
		}
	...
	setReadCellColor : function(line, colindix, row, rowspan) {
		var color = Hq.grid.DataGrid.defaultCellColor;
		if (rowspan != null) {
			color = rowspan(line, colindix, row,this.setting.dataTable.getCols()[colindix]);
		}
		if (color == Hq.grid.DataGrid.defaultCellColor) {
			...
			//return Hq.grid.DataGrid.defaultCellColor;
		}
		return color;
	...
	drawPanel : function() {
		Hq.grid.DataGrid.superclass.drawPanel.call(this);
		var config = this._getPanelConfig();
	...
	_hrefColFormat : function(event) {
		...
		if (event.hqDataTable.setting.headTotal && event.rowIndex == 0) {
			return value;
		}
		if (event.currentCol.xType === "fileuploadfield"
			...
			var text = event.currentRow[Hq.grid.DataGrid.PREFIXEXPAND
					+ currentCol.name];
			text = text || "";
			if (text.length == 0) {
				return "";
	...
	_fileuploadfieldFormat : function(event) {
		...
		var text = event.currentRow[Hq.grid.DataGrid.PREFIXEXPAND + currentCol.name];
	...
	_dataTable_cellfileuploadfield : function(input, currentRow, currentCol,
		...
		var uploadFileUrl = currentCol.uploadFileUrl 	|| Hq.grid.DataGrid.UPLOADFILEURL;
		function uploadFile(attachmentContainer, limitColumn, limitSize, storeFilePath) {
			...
			var fileUpload = {
				...
				data_url : data_url,
				startBtn : 'Hq_grid_DataGrid_startBtn'
			};
			if($("#"+fileUpload.tableID).length  > 0){
				$("#"+fileUpload.tableID).remove();
			}
		   var render = template.compile(Hq_Modules_uploadFile_html);
		   var uploadFileDialogDivHTML = render(fileUpload);
		   Hq.create({
				xType : "dialog",
				title : "上传文件", // 标题. 默认'提示'
				id : "Hq_grid_DataGrid_FileUpload_window",
				autoOpen : true,
				content : $(uploadFileDialogDivHTML).appendTo(document.body),
				button : [{
							name : "关闭",
							handler : function(obj) {
								Hq.getCmp("Hq_grid_DataGrid_FileUpload_window").close();
								Hq.getCmp("Hq_grid_DataGrid_FileUpload_window").destroy();
								if ($("#" + fileUpload.tableID).length > 0) {
									$("#" + fileUpload.tableID).remove();
								}
							}
			...
			$('#Hq_grid_DataGrid_closeBtn').click(function() {
						var dialog = Hq.getCmp("Hq_grid_DataGrid_FileUpload_window");
						dialog.close();
						dialog.destroy();
						if ($("#" + fileUpload.tableID).length > 0) {
							$("#" + fileUpload.tableID).remove();
			...
			$('#uploadfile-input-form').fileupload({
				...
				add : function(e, data) {
					...
					$("#Hq_grid_DataGrid_startBtn").unbind("click");
					$("#Hq_grid_DataGrid_startBtn").attr("disabled", false).click(function() {
								$(this).text('上传文件中...');
								data.submit();
								$("#Hq_grid_DataGrid_startBtn").attr("disabled", true);
							}).text('开始上传');

				},
				done : function(e, data) {
					$("#Hq_grid_DataGrid_startBtn").attr("disabled", false);
					if (data.result.success == true) {
						...
						Hq.Msg.alert(data.result.erroInfo);
						$("#Hq_grid_DataGrid_startBtn").text('请重新选择文件');
						return;
					}
		...
		$(input).bind("Event.AttachmentUploadSuccess",
				function(event, attachmentID, attachmentName) {
					var data = {};
					data[Hq.grid.DataGrid.PREFIXEXPAND + currentCol.name] = attachmentName;
	...
	removeListener : function(eventName, handler, col) {
		...
		Hq.EventManager.removeEventListener({
					eventType : selector,
					handler : handler,
					target : this
				});
	...
	initPlugin : function() {
		...
		if (this.setting.isExpression == true  &&  this.getIsExpression() == true) {
			...
			if (this.isGroupTable() && this.setting.plugins.length  == 0 ) {
				this.setting.plugins = [Hq.DataTableGroupColsExpression];
			} else {
				this.setting.plugins = [Hq.DataTableNormalColsExpression];
			}
		}
		...
		for (var i = 0; i < components.length; i++) {
			if ($.type(components[i]) === "function") {
				...
				if ("init" in comp) {
					...
					Hq.Logger.info("插件 error " + comp.toString());
				}
			}
		}
	},
	...
	batchCalculateExp : function(){
		 ...
		 */
		var url = this.setting.basePath + Hq.DataTableNormalColsExpression.BATCHCALCULATEEXP;
		...
		params["rows"] = Hq.JSON.stringify(refDatas);
		var me = this;
		$.ajax({
			url : url,
			type : 'post',
	...
	sumCols : function(colName) {
		...
		if (this.setting.headTotal && col.sum == true) {
			...
			for (var j = 0; j < allDatas.length; j++) {
				...
				oriSum = Hq.tools.accAdd(oriSum, allDatas[j][colName] || 0);
			}
			var tempData = {};
			tempData["_sortid"] = 0;
	...
	_setGroupInfo : function(){
		...
		//为Hq.create(setting)重新处理一下setting
	...
	_setGroupInfoTmp : function(){
		...
		if (groupSels && sortSels && groupSels.length > 0 && sortSels.length > 0) {
			for (var i = 0; i < groupSels.length; i++) {
				...
				})(this.groupColsInfoTmp, Hq.tools.query({"columnID" : groupSels[i].value},groupCols));
			}
		}
	},
		...
		Hq.pubAjax({
					...
					success : function(_setting) {
						...
						if ($.type(setting.callback.Comparable) === 'function') {
							var newData = Hq.tools.toJSON(_setting.columns,
									_setting.datas);
							newData.sort(setting.callback.Comparable);
							_setting.datas = Hq.tools.toARRAY(_setting.columns, 	newData);
						...
						newEvent.eventType = Hq.BaseEvent.eventType.LOADDATA;
						...
						if (setting.defalutChecked != null) {
							...
							if ($.type(setting.defalutChecked) == "boolean") {
								...
								for (var i = 0; i < rs.length; i++) {
									var tempData = Hq.tools.query( setting.defalutChecked, [rs[i]])
	...
	queryColumn : function(fql) {
		...
		for (var i = 0; i < element.length; i++) {
			var e = element[i];
			var temp_e = Hq.tools.queryContain(fql, e);
			if (temp_e) {
				list.push(temp_e);
			}
		}
	...
	reloadTableData : function(array, columns) {
		...
		if (!$.isArray(array[0])) {
			array = Hq.tools.toARRAY(c, array);
		}
	...
	analyseKey : function(opradata) {
		 ...
		 *//**
						 ...
						 * 
						 * var temp = Hq.tools.query(fql, realUpdateData); //
...
//			}
		...
		for (var i = 0; i < cols.length; i++) {
			if (cols[i].display == true) {
				if ("defaultValue" in cols[i]) {
					...
					if ($.type(cols[i].csId) == "string") {
						tempdata[Hq.grid.DataGrid.PREFIXEXPAND_C + cols[i].name] = cols[i].defaultShowValue;
					}
				} else {
					tempdata[cols[i].name] = null;
					if ($.type(cols[i].csId) == "string") {
						tempdata[Hq.grid.DataGrid.PREFIXEXPAND_C + cols[i].name] = null;
					}
				}
			}
		}
	...
	getDefaultData : function() {
		...
		if (result.status == 200 && result.readyState == 4 && result.responseText) {
			relustData = Hq.JSON.parse(result.responseText);
...
//		}
		...
		if ($.type(data._locationposition) === "number") {
			...
			if (delData["C_STATUS"] != "1") {
				...
				if (this.setting.getAllUpdate == false) {
					...
					this.delList.push(Hq.merge({}, delData));
				}
	...
	saveData : function(params, url, fn) {
		...
		grid.pagination = this.setting.pagination;
		var json = Hq.JSON.stringify(grid);
		if (!($.type(url) === "string")) {
		...
		Hq.ajax({
					...
					success : function(json) {
						...
						Hq.EventManager
								.dispatchEvent(new Hq.grid.DataGridEvent({
									eventElement : setting.me,
									eventType : Hq.BaseEvent.eventType.SAVEDATA,
									hqDataTable : setting.me,
	...
	destroy : function() { 
		...
		// 删除Fbar
	  	Hq.grid.DataGrid.superclass.destroy.call(this);
		...
		this.setting = null;
		delete this.setting;
		Hq.Logger.info("DataTable is destroy");
	},
	/**
	 * 得到引用字段
	 */
	...
	_processEvent : function(event) {
			...
			if (srcElement.nodeName === 'A' && event.domInnerText === '下载') {
				if (currentValue != null && currentValue.length > 0) {
					var downloadFileUrl = currentCol.downloadFileUrl
							|| Hq.grid.DataGrid.DOWNLOADFILEURL;
		...
		var params = Hq.tools.toParams(arguments);
		var newEvent = new Hq.grid.DataGridEvent({
					eventElement : me,
					eventType : Hq.BaseEvent.eventType.EDITSTART,
					hqDataTable : me,
		...
		var editstartFilter = Hq.EventManager.dispatchEvent(newEvent);
		...
		//大文本，即使返回false也弹出大文本对话框
		    if (currentCol.xType == "textarea") { 
					...
					Hq.Logger.info(this.id + "grid停止编辑:业务控制 编辑前事件");
			}
		     return editstartFilter;
		}
		var row = Hq.JSON.stringify(currentRow);
		/*if (currentCol.needRow == true) {
			row = Hq.JSON.stringify(currentRow);
		}*/
		...
		if (reCol.length > 0) {
			for (var k = 0; k < reCol.length; k++) {
				if (currentRow[reCol[k].name] == null || currentRow[reCol[k].name] == undefined || currentRow[reCol[k].name] == '') {
					Hq.Msg.alert("请先选择" + (reCol[k].alias2 ==null?reCol[k].alias:reCol[k].alias2 ) + "数据")
					return;
				}
			...
			if (currentCol.xType == "combo" 
				...
				var treePnael = $(cellobj).treePanel({
					...
					params : {
						...
						value : value,
						format : "ztree",
						params : $.type(treePnaelParams)=='object'?Hq.encode(treePnaelParams):treePnaelParams,
						row : row
					},
					...
					callback : {
						onClick : function(event, treeId, treeNode) {
							...
							if (!currentCol.checkAll) {
								if (!treeNode.isParent) {
									data[Hq.grid.DataGrid.PREFIXEXPAND + currentCol.name] = treeNode.name;
									data[currentCol.name] = treeNode.id;
									me.setBindCol(currentRow, currentCol, data, treeNode);
									Hq.EventManager.dispatchEvent($.extend(
											newEvent, {
						...
						clear : function() {
							...
							if (currentCol.isNullable != null && !currentCol.isNullable) {
								Hq.Msg.alert(currentCol.alias2+"不能为空");
								return;
							}
							data[Hq.grid.DataGrid.PREFIXEXPAND + currentCol.name] = "";
				...
				if (srcElement.nodeName === 'A' && srcElement.innerText === '删除') {
					Hq.MessageBox.confirm("是否确定删除？",function() {
							...
							var a = Hq.EventManager.dispatchEvent(new Hq.grid.DataGridEvent({
												eventElement : me,
				...
				Hq.pubAjax({
					...
					success : function(json) {
						if(json.length > 0){
							...
							//只能有一个叶子节点
							if(tempLeafCount  === 1){
								data_bak[Hq.grid.DataGrid.PREFIXEXPAND + relaColArray[i].name] = json[LeafIndex].name;
		...
		if (vdata) {
			...
			if (currentCol.csId != null) {
				if (currentCol.csId.length > 0) {
					data[Hq.grid.DataGrid.PREFIXEXPAND_C + currentCol.name] = currentCol.defaultShowValue;
				}
			} 
	...
	_getFootBarPanel:function(config){
	   ...
	   return  Hq.grid.DataGrid.superclass._getFootBarPanel.call(this); 
	},
	addFootButton:function(fbar){
	   this.fbar=Hq.grid.DataGrid.superclass.addFootButton(fbar);
	},
	...
	setWidth : function(w) { 
		...
		// 状态栏区
		var newEvent = new Hq.Panel.PanelEvent({
					panel : this,
					async : true,
					width : w,
					type : "width",
					...
					async : true,
					eventElement : this,
					eventType : Hq.BaseEvent.eventType.RESIZE
				});
		Hq.EventManager.dispatchEvent(newEvent); 
		}
	},
	setHeight : function(h) {
		if ($.type(h) == 'number') {
			var config = this._getPanelConfig();
			Hq.grid.DataGrid.superclass.setHeight.call(this, h);
			this.setting.dataTable.resize(this.getContentWidth()-2, this.getContentHeight()-2);
		} 
	},
	/**
	...
	resetContentHeight : function() {
		Hq.grid.DataGrid.superclass.resetContentHeight.call(this);
		// 修改当前渲染区的DIV 的高度
		this.setting.dataTable.resize(this.getContentWidth(), this.getContentHeight());
	}
});
	
	Hq.override(Hq.grid.DataGrid, {
				isPrefixExpand : function(fieldName) {
					var regex = new RegExp("^" + Hq.grid.DataGrid.PREFIXEXPAND);
					return regex.test(fieldName);
				},
				isPrefixExpand_C : function(fieldName) {
					var regex = new RegExp("^" + Hq.grid.DataGrid.PREFIXEXPAND_C);
					return regex.test(fieldName);
				}
			});

// 注册到xType 管理器
Hq.reg(Hq.grid.DataGrid);
